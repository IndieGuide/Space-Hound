...
if place_meeting(x+speed_[h],y,o_solid) && stop_by_solid_flag_ {
	while !place_meeting(x+sign(speed_[h]),y,o_solid){
		x+=sign(speed_[h]);
	}
	
	if bounce_x>0{
		speed_[@ h]=-speed_[@ h]*bounce_x;
	}else{
		speed_[@ h]=0;
	}
}
...
if place_meeting(x,y+speed_[v],o_solid) && stop_by_solid_flag_ {
	while !place_meeting(x,y+sign(speed_[v]),o_solid){
		y+=sign(speed_[v]);
	}
	
	if bounce_y>0{
		speed_[@ v]=-speed_[@ v]*bounce_y;
	}else{
		speed_[@ v]=0;
	}
}
